...
export default function RequestAccessForm() {
  ...
  const loadRequesterForm = () => {
    if (!Array.isArray(formData) || formData.length === 0 || !formData[0].content?.selected) {
      alert('Form data is invalid or missing.');
      return;
    }

    const selectedFromForm = formData[0].content.selected;

    setStoredForm({ selected: selectedFromForm });

    const matched = matchFormSections(selectedFromForm);
    ...
    Object.keys(selectedFromForm).forEach((code) => {
